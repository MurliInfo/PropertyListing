...
class SLPBedrooms: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        MAX = appDelegate.FILTERBEDROOMSMAX
        MIN = appDelegate.FILTERBEDROOMSMIN
        
        if appDelegate.FILTERBEDROOMSMIN != "Any"{
            for  i in 0..<arrayMinBedroom.count{
                if appDelegate.FILTERBEDROOMSMIN as String == arrayMinBedroom[i]{
                    min = i
                }
            }
            self.pickerViewData.selectRow(min, inComponent: 0, animated: true)
        }
        if appDelegate.FILTERBEDROOMSMAX != "Any"{
            for  i in 0..<arrayMinBedroom.count{
                if appDelegate.FILTERBEDROOMSMAX as String == arrayMaxBedroom[i]{
                    max = i
                }
            }
    ...
    @IBAction func Done(_ sender: Any) {
        appDelegate.FILTERBEDROOMSMAX = MAX
        appDelegate.FILTERBEDROOMSMIN = MIN
//        appDelegate.FILTERBEDROOMSMAX = arrayMaxBedroom[pickerViewData.selectedRow(inComponent: 2)] as NSString
//                appDelegate.FILTERBEDROOMSMIN = arrayMinBedroom[pickerViewData.selectedRow(inComponent: 0)] as NSString
...
extension SLPBedrooms : UIPickerViewDataSource,UIPickerViewDelegate{
    @available(iOS 2.0, *)
